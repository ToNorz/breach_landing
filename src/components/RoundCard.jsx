  ...
  return (
    <div className="bp-panel rounded-sm p-8 md:p-10 flex flex-col h-full">
      ...
      <div className="flex items-center justify-between mb-8">
        ...
        <span
          className={`bp-mono text-[10px] uppercase px-2.5 py-1 rounded-sm ${
      ...
      <div className="text-sm md:text-base leading-relaxed" style={{ color: "var(--text-dim)" }}>
          ...
          <>
            {description.split(highlightText).map((part, i, arr) => (
              <React.Fragment key={i}>
                {part}
                {i < arr.length - 1 && (
                  <strong style={{ color: "var(--text)" }}>{highlightText}</strong>
                )}
              </React.Fragment>
            ))}
          </>
